...
lib.mkIf osConfig.lcars.user.hyprland.enable {
  wayland.windowManager.hyprland = {
    ...
    settings = {
      ...
      #
      # Barra, notificações, papel de parede e tela de bloqueio sobem todos
      # dentro do próprio noctalia (programs.noctalia.systemd, em
      # user/wm/noctalia.nix) — um processo, uma unidade systemd, que
      # reinicia o que cai e responde a `systemctl --user status`; pelo
      # exec-once, falha é silêncio.
      #
      ...
      # quando o alvo hyprpaper está habilitado — e este repo não o liga
      # (system/wm/hyprland.nix explica o porquê: conflito com o mpvpaper do
      # noctalia). Sem elas, o logo do Hyprland ficaria desenhado por cima da
      # cor de fundo.
      misc = {
